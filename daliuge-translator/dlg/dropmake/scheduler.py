                ...
                for i in range(elcpu):
                    ...
                    for uup in el_pred:
                        for j in range(get_nb_cpus(uup)):
                            self_bpg.add_edge("{0}{1}_l".format(uup, j), child_r)
                    ...
                    for udown in el_des:
                        for k in range(get_nb_cpus(udown)):
                            self_bpg.add_edge(child_l, "{0}{1}_r".format(udown, k))
...
class MultiWeightPartition(Partition):
    ...
    def __init__(self, gid, max_dops, w_attrs=["num_cpus"], global_dag=None):
    ...
    def _add_to_tc(self, tc, el, dag, tmp_dag_list):
        ...
        if self._check_global_dag:
            ...
            global_dag = self._global_dag
            for rel in rem:
                if (not rel in tc[el]) and nx.has_path(global_dag, el, rel):
            ...
            for rel in rem:
                if (not el in tc[rel]) and nx.has_path(global_dag, rel, el):
...
class MCTSScheduler(PSOScheduler):
    ...
    def partition_dag(self):
        ...
        mcts = MCTS(
            stree, calculation_time=self._max_calc_time, max_moves=self._max_moves
        )
        ...
        if logger.isEnabledFor(logging.DEBUG):
            leng = len(G.edges())
            logger.debug(
                "Each MCTS move on average took %.2f seconds",
                (time.time() - stt) / leng,
            )
        ...
        edt = time.time()
        logger.debug(
            "Monte Carlo Tree Search scheduler took %f secs, lpl = %d, num_parts = %d",
            edt - stt,
            curr_lpl,
            num_parts,
        )
...
class SAScheduler(PSOScheduler):
    ...
    def __init__(
        self, drop_list, max_dop=8, dag=None, deadline=None, topk=None, max_iter=6000
    ):
        """
    ...
    def partition_dag(self):
        ...
        mys = MySarkarScheduler(
            self._drop_list, max_dop=self._max_dop, dag=self._lite_dag.copy()
        )
        ...
        edt = time.time()
        logger.debug(
            "Simulated Annealing scheduler took %f secs, energy = %f, num_parts = %d",
            edt - stt,
            e,
            num_parts,
        )
